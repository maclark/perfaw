...
static void PrintValue(char const *Label, repetition_value Value, u64 CPUTimerFreq)
{
    ...
    {
        E[EIndex] = Value.E[EIndex] / Divisor;
    }
    ...
    {
        printf(" PF: %0.4f (%0.fk/fault)", E[RepValue_MemPageFaults], E[RepValue_ByteCount] / (E[RepValue_MemPageFaults] * 1024.0));
    }
}
...
static void NewTestWave(repetition_tester *Tester, u64 TargetProcessedByteCount, u64 CPUTimerFreq, u32 SecondsToTry = 10)
{
    if (Tester->Mode == TestMode_Uninitialized)
    {
        ...
        Tester->Results.Min.E[RepValue_CPUTimer] = (u64)-1; // hm isn't -1 going to be super big?
    }
        ...
        if(Tester->OpenBlockCount) // casey says we ignore timeblocks that took another path
        {
            ...
            if(Tester->Mode == TestMode_Testing)
            {
                ...
                if(Results->Min.E[RepValue_CPUTimer] > Accum.E[RepValue_CPUTimer])
                {
                    ...
                    // casey says whenever we get a minimum time, we reset the time to the full trial time
