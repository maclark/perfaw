...
static void NOPAllBytes(repetition_tester *Tester, read_parameters *Params)
{
    while(IsTesting(Tester))
    {
        ...
        BeginTime(Tester);
        NOPAllBytesASM(DestBuffer.Count, DestBuffer.Data);
        EndTime(Tester);
...
static void CMPAllBytes(repetition_tester *Tester, read_parameters *Params)
{
    while(IsTesting(Tester))
    {
        ...
        BeginTime(Tester);
        CMPAllBytesASM(DestBuffer.Count, DestBuffer.Data);
        EndTime(Tester);
...
static void DECAllBytes(repetition_tester *Tester, read_parameters *Params)
{
    while(IsTesting(Tester))
    {
        ...
        BeginTime(Tester);
        DECAllBytesASM(DestBuffer.Count, DestBuffer.Data);
        EndTime(Tester);
